...
function routeFor(
  from: string,
  to: string,
  peers: Map<string, Instance<typeof PeerModel>>,
) {
...
const MeshNetworkModel = types
  ...
  .views((self) => ({
    ...
    get deliveredPackets() {
      return self.packets.filter((packet) => packet.status === "delivered")
        .length;
    },
  }))
  .actions((self) => {
    ...
    const attachPeerListener = (peerId: string) => {
      const listener: EventListener = async (event) => {
        ...
        (
          self as typeof self & { finalizeDelivery: DeliveryHandler }
        ).finalizeDelivery(peerId, detail, body);
      };
    ...
    const sendSecureMessage = flow(function* sendSecureMessage(
      from: string,
      to: string,
      body: string,
    ) {
      ...
      sender.markSent();
      remember(
        `${sender.label} queued encrypted packet for ${recipient.label}`,
      );
