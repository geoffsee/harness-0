  ...
  return (
    <>
      <BaseEdge
        ...
        path={path}
        markerEnd={markerEnd}
        style={{ stroke: data?.stroke, strokeWidth: 3.5 }}
      />
      {data?.label ? (
        <EdgeLabelRenderer>
          <div
    ...
    >
      ...
      >
        <div>
          ...
          <div style={{ marginTop: 3, fontSize: 18, fontWeight: 700 }}>
            {data.label}
          </div>
        </div>
        <div
...
const peerMeta: Record<
  string,
  { position: { x: number; y: number }; role: string }
> = {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      await meshNetwork.sendSecureMessage(
        "alpha",
        "gamma",
        "Ephemeral session key rotated at the edge.",
      );
      await meshNetwork.sendSecureMessage(
        "delta",
        "beta",
        "Forwarding signed topology update.",
      );
      await meshNetwork.sendSecureMessage(
        "gamma",
        "alpha",
        "Acknowledged over secure event channel.",
      );
    })();
              ...
              <Panel position="bottom-right">
                <div style={legendPanelStyle}>
                  <div style={legendRowStyle}>
                    <span
                      style={{ ...legendSwatchStyle, background: "#4dd0a8" }}
                    />
                    Delivered route
                  </div>
                  <div style={legendRowStyle}>
                    <span
                      style={{ ...legendSwatchStyle, background: "#294252" }}
                    />
                    Mesh link
                  </div>
                </div>
            ...
            <div style={{ display: "grid", gap: 6, marginTop: 8 }}>
              {graph.packets.map((packet) => (
                <div key={packet.id} style={denseStatusRowStyle}>
                  ...
                  <div style={statusBadgeStyle(packet.status)}>
                    {packet.status}
                  </div>
                </div>
              ))}
            ...
            <ul style={{ ...listStyle, gap: 6, marginTop: 8 }}>
              {graph.packets.map((packet) => (
                <li key={packet.id} style={denseListRowStyle}>
                  ...
                  <div style={denseSubtleStyle}>
                    Digest {packet.digest.slice(0, 16)}...
                  </div>
                  <div style={denseSubtleStyle}>
                    Route {packet.route.join(" -> ")}
                  </div>
                </li>
              ))}
            </ul>
...
const statusBadgeStyle = (status: string): React.CSSProperties => ({
  ...
  background:
    status === "delivered"
      ? "rgba(41, 161, 116, 0.12)"
      : "rgba(198, 141, 39, 0.12)",
